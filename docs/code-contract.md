...
- Local database schema：`packages/contracts/src/localDatabase.ts`
- 免费公开题库：`packages/content/src/public/questions.ts`
- Phase 1 遗留浏览器存储边界：`apps/web/src/features/scorecard/storage.ts`

Question、Scorecard 和本地数据库字段以 `packages/contracts/src/*.ts` 为唯一事实源，文
档不重复列字段。

任何 app（当前 `apps/web`，未来 `apps/api`、`apps/miniapp`）都不能重新定义
`Question`、`Scorecard`、本地数据库类型、`questionSchema` 或 `scorecardSchema`.

Phase 2 的个人学习数据目标固定为本地目录：
...
`database/` 是未来可落库的核心数据；`derived/` 是可重建索引；`backups/` 和
`corrupted/` 只做兜底。内置题库仍属于 `packages/content`，用户学习数据只引用
`questionId`，不复制题目正文和答案。
...
`packages/domain` 只放业务纯函数。它不能依赖 Vue、Naive UI、router、浏览器存储或本
地文件系统。

Phase 1 遗留浏览器本地 scorecard 存储只能放在
`apps/web/src/features/scorecard/storage.ts`。Phase 2 新增核心学习数据不得继续写入
`localStorage`；后续必须通过本地 Node API 写入固定目录。页面和组件不直接访问
`localStorage` 或本地文件系统。
...
- 题库内容必须是 free-only
- `localStorage` 只能出现在 Phase 1 遗留
  `apps/web/src/features/scorecard/storage.ts`
- 本地数据库目录和记录不能进入 git 可见文件
